...
\[L(p;x)=\frac{20!}{x!(20-x)!}p^x(1-p)^{20-x}\]
...
\[L(\lambda:y)=\frac{e^{-n\lambda}\lambda^{\sum^n_t=0y_t}}{x_1!x_2!\cdot \cdot \cdot x_n!}\]
\[l(\lambda:y)=log(L(\lambda:y))=log(e^{-n\lambda})+log(\lambda^{\sum^n_{t=0}y_t})\]
\[l(\lambda:y)=-n\lambda log(e)+\sum^n_{t=0}y_tlog(\lambda)=\sum^n_{t=0}y_tlog(\lambda)-n\lambda\]
...
\[l(\theta_1|\theta_2)=\sum^n_{t=0}y_tlog(\lambda)-n\lambda-2log(x!)+log(n!)-log((n-x)!)+xlog(p)+(n-k)log(1-p)\]
